...
public class LinctexServlet extends AbstractRobot {
	...
	public void onDocumentChanged(DocumentChangedEvent event) {
		...
		if (event.getModifiedBy().startsWith("spelly") ||
			event.getModifiedBy().startsWith("panda")) {
			return;
		}
			...
			if (e != null && e.isImage()) {
				...
				if (url != null && url.startsWith("http://www.forkosh.")) {
					...
					if (mode == Mode.SYNTAX) {
						...
						ref = BlipContentRefs.range(blip, i, i
								+ formula.length());
						ref.annotate("robot.linctex", "formula");
						ref.annotate("style/backgroundColor",
								"rgb(200,200,200)");
					}
				}
			}
